    ...
    list.forEach(p => {
      ...
      <div class="b">
        ...
        el.onclick = (e) => {
            ...
            if (target.closest(".acts") || target.closest(".ptsrow"))
                return;
        ...
        const inp = el.querySelector(".ptsrow input");
        ...
        const qs = el.querySelector(".qsel");
        ...
        el.querySelector(".bfoto").onclick = (e) => {
            e.stopPropagation();
            pickPhoto(p);
        };
        el.querySelector(".blink").onclick = (e) => {
            e.stopPropagation();
            pickLink(p);
        };
        el.querySelector(".bfile").onclick = (e) => {
            e.stopPropagation();
            pickFile(p);
        };
        const rem = el.querySelector(".brem");
        if (rem) {
            rem.onclick = (e) => {
                e.stopPropagation();
...
function pickFile(p) {
    ...
    inp.onchange = () => {
        ...
        const r = new FileReader();
        r.onload = () => {
            p.file = { name: f.name, data: r.result };
            renderGrid();
            saveState();
        };
        r.readAsDataURL(f);
...
function openHelp(key) {
    const h = HELP[key];
    if (!h || !hmodal || !hmTitle || !hmBody)
        return;
    hmTitle.textContent = h.title;
    hmBody.innerHTML = h.items.map(it => `<div class="hitem"><b>${it.b}</b><p>${it.p}</p>${it.ex ? `<div class="ex">${it.ex}</div>` : ""}</div>`).join("");
...
if (hmClose)
    hmClose.onclick = () => hmodal?.classList.remove("show");
if (hmodal) {
    hmodal.onclick = (e) => {
        if (e.target === hmodal)
...
const amEmo = document.getElementById("amEmo");
const amLink = document.getElementById("amLink");
const amPhoto = document.getElementById("amPhoto");
const amFile = document.getElementById("amFile");
const amQty = document.getElementById("amQty");
function openAdd() {
    if (amErr)
    ...
    if (amQty)
        amQty.value = "ind";
    document.querySelectorAll("#amSegs input").forEach(c => c.checked = false);
...
if (amClose)
    amClose.onclick = closeAdd;
const amCancel = document.getElementById("amCancel");
if (amCancel)
    amCancel.onclick = closeAdd;
if (amodal) {
    amodal.onclick = (e) => {
        if (e.target === amodal)
            closeAdd();
...
if (amSave) {
    amSave.onclick = async () => {
        ...
        const segs = Array.from(document.querySelectorAll("#amSegs input:checked")).map(c => c.value);
        if (!name || !segs.length) {
            if (amErr)
        ...
        LIB.push(p);
        selected.add(p.id);
        if (activeSeg !== "ALL" && !segs.includes(activeSeg))
            activeSeg = "ALL";
        closeAdd();
        buildTabs();
        renderGrid();
...
function renderTrilhaPanel() {
  ...
  `;
    const objI = document.getElementById("tpObjetivo");
    if (objI) {
...
document.querySelectorAll('input[name="mode"]').forEach(r => {
    r.addEventListener("change", () => {
        document.querySelectorAll(".mode").forEach(m => {
            const rb = m.querySelector("input");
...
function calcGuide() {
    ...
    if (gTiers) {
      ...
      <div class="tierrow">
        ${tiers.map(t => `<div class="tier"><span class="tn">${t[0]}</span><span class="tp">${Math.max(1, Math.round(ptsCiclo * t[1]))} pts</span></div>`).join("")}
      </div>
...
document.querySelectorAll(".navbtn").forEach(b => {
    b.addEventListener("click", () => {
        ...
        views.forEach(v => {
            ...
            if (el)
                el.style.display = v.toLowerCase().includes(view || "") ? "" : "none";
        });
        window.scrollTo(0, 0);
        if (view === "guide") {
...
function getConfig() {
    ...
    const fmtChecked = document.querySelector('input[name="fmt"]:checked');
    return {
        mode: currentMode(),
...
async function openOutput() {
    ...
    try {
        ...
        if (btnOther)
            btnOther.innerHTML = isPdf ? "🌐 Baixar site (HTML)" : "📄 Salvar como PDF";
...
async function fetchOrders() {
    if (!currentActiveSlug || !tbodyOrders)
        return;
    try {
